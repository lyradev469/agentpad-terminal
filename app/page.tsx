    ...
    try {
      ...
      try {
        ...
        for (const log of eventLogs) {
          try {
            // Manual decoding of TokenLaunched event
            // Event: TokenLaunched(address indexed tokenAddress, address indexed poolAddress, string name, string symbol)
            // Topics: [eventSelector, tokenAddress, poolAddress, nameHash, symbolHash]
            // For indexed addresses, topics[1] = tokenAddress, topics[2] = poolAddress
            
            if (log.topics.length >= 3) {
              tokenAddress = log.topics[1] as `0x${string}`
              poolAddress = log.topics[2] as `0x${string}`
              break
            }
          } catch (e) {
      ...
      switch (command) {
        case 'help':
          addCommand(trimmed, [
            ...
            '',
            'Wallet Requirements:',
            '  - launch, claim, fees, pools require connected wallet',
            '  - Use the Connect Wallet button or run "connect"',
            '',
            'Examples:',
        ...
        case 'pools': {
          if (!isConnected) {
            addCommand(trimmed, [
              '❌ Error: Wallet not connected',
              '   Please click "Connect Wallet" button or run "connect"',
            ], true)
            break
          }
          if (parts.length < 2) {
        ...
        case 'status': {
          const statusLines = [
            ...
            `  USDC:     ${CONFIG.pathUSD}`,
            ``,
            isConnected ? `   Run "launch", "claim", "pools" to interact` : `   Use "Connect Wallet" button to start`,
            ``,
          ]
